...
export async function clientLoader() {
  const res = await fetch(
    "https://restcountries.com/v3.1/all?fields=languages,capital,area,borders,continents,currencies,name,region,subregion,population"
  );
...
export default function Countries({ loaderData }: Route.ComponentProps) {
  ...
  const filteredSearch = loaderData.filter((country: any) => {
    ...
    return matchedSearch && regionSearch;
  });
    ...
    <div className="p-6">
      ...
      {filteredSearch.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredSearch.map((country: any) => (
            <li
              key={country.cca3}
